        ...
        <div className="card bg-neutral drop-shadow-lg text-blue-200 w-96 bg-gradient-to-br inline-block from-indigo-950 to-slate-500">
            <div className="card-body items-left text-left">
                <div className="flex justify-between">
                    ...
                    <img className="w-10 h-10 rounded-full drop-shadow-lg object-cover" src={import.meta.env.VITE_BASE_URL + `${import.meta.env.VITE_BASE_PIC}${teamPicPath}`} alt="Team Profile" />
                </div>

                <div className="flex justify-evenly">
                    <p className="drop-shadow-lg text-stone-300 font-extrabold">Rövid név:</p>
                    <p className="drop-shadow-lg">{team.short_name}</p>
                </div>

                <div className="flex justify-evenly">
                    <p className="drop-shadow-lg text-stone-300 font-extrabold">Csapat tagjai:</p>
                    <div>
                        {teamMembers.length === 0 ? (
                            <p className="text-stone-300">Nincsenek aktív tagok</p>
                        ) : (
                            teamMembers.map(member => (
                                <div key={member.id} className="text-amber-500 hover:text-indigo-900">
